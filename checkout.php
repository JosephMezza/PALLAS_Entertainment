...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($fname_err) && empty($lname_err) && empty($email_err) && empty($card_number_err) && empty($cardholder_err) && empty($street_address_err) && empty($expiration_date_err) && empty($cvv_err) && empty($city_err) && empty($zip_err)) {
        ...
        $id++;
        $subject = "PALLAS Ticket Confirmation";
        $headers = "From: PALLAS";
        $body = "Thank you for purchasing a ticket with PALLAS Entertainment!\n
    ...
    // Close connection
    mysqli_close($link);
    header('Location:https://pallas.website/mainpage.php?success=1');
    ob_end_flush();
}
